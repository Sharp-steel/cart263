...
let rectangleObject = new RectangularObj(100,100,50,70,"#FF5733","#E6E6FA",drawingBoardB.context);
//add a rectangular object to canvas B
drawingBoardB.addObj(rectangleObject);
...
getMicrophoneInput(rectangleObject);

}

async function getMicrophoneInput(rectangleObject) {
  ...
  try {
    ...
    function audioAnalyze() {
      ...
      let sum = 0;
      // console.log("audio analyze");
 
      for (let i = 0; i < bufferData.length; i++) {
        sum += bufferData[i];
      }
      let averageVol = sum / bufferData.length;
      //console.log(averageVol);
      rectangleObject.setVolume(averageVol);
      
      requestAnimationFrame(audioAnalyze);
    }

    audioAnalyze();
  } catch (err) {
    /* handle the error */
    console.log("The microphone didn't work :(");
  }
}
